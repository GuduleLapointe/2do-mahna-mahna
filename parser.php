...
function hostScan($hostname, $port, $xmlcontent)
{
	...
	$gatekeeperURL = !empty($objDOM->getElementsByTagName("gatekeeperURL"))
		? $objDOM->getElementsByTagName("gatekeeperURL")->item(0)->nodeValue
		: "";
	...
	$query = $SearchDB->prepare(
		"UPDATE hostsregister SET nextcheck = ?, gatekeeperURL = ? WHERE host = ? AND port = ?",
	);
	$query->execute([$next, $gatekeeperURL, $hostname, $port]);
	...
	foreach ($regionlist as $region) {
		...
		$regionhandle = $info->getElementsByTagName("handle")->item(0)
			->nodeValue;
		...
		$SearchDB->insert(SEARCH_REGION_TABLE, [
		...
		foreach ($parcels as $parcel) {
			$parcelname = $parcel->getElementsByTagName("name")->item(0)
				->nodeValue;
			$parcelUUID = $parcel->getElementsByTagName("uuid")->item(0)
				->nodeValue;
			$infoUUID = $parcel->getElementsByTagName("infouuid")->item(0)
				->nodeValue;
			$landingpoint = $parcel->getElementsByTagName("location")->item(0)
				->nodeValue;
			$parceldescription = $parcel
				->getElementsByTagName("description")
				->item(0)->nodeValue;
			$parcelarea = $parcel->getElementsByTagName("area")->item(0)
				->nodeValue;
			...
			$ownerUUID = $owner->getElementsByTagName("uuid")->item(0)
				->nodeValue;

			// Adding support for groups
			$group = $parcel->getElementsByTagName("group")->item(0);
			if ($group) {
				$groupUUID = $group->getElementsByTagName("groupuuid")->item(0)
					->nodeValue;
			}
			if (empty($groupUUID)) {
			...
			$parceldirectory = $parcel->getAttributeNode("showinsearch")
				->nodeValue;
			...
			$query = $SearchDB->prepare(
				"DELETE FROM popularplaces WHERE parcelUUID = ?",
			);
			$query->execute([$parcelUUID]);
			...
			$query = $SearchDB->prepare(
				"DELETE FROM allparcels WHERE parcelUUID = :parcelUUID",
			);
			$query->execute(["parcelUUID" => $parcelUUID]);

			$SearchDB->insert("allparcels", [
				"regionUUID" => $regionUUID,
		...
		foreach ($objects as $object) {
			$objectname = $object->getElementsByTagName("title")->item(0)
				->nodeValue;
			$location = $object->getElementsByTagName("location")->item(0)
				->nodeValue;
			$parcelUUID = $object->getElementsByTagName("parceluuid")->item(0)
				->nodeValue;
			$regionUUID = $object->getElementsByTagName("regionuuid")->item(0)
				->nodeValue;
			...
			$SearchDB->insert("objects", [
				"objectuuid" => $object->getElementsByTagName("uuid")->item(0)
					->nodeValue,
				"parceluuid" => $parcelUUID,
				"location" => $location,
				"name" => $location,
				"description" => $object
					...
					->item(0)->nodeValue,
				"regionuuid" => $object
					->getElementsByTagName("regionuuid")
					->item(0)->nodeValue,
				"gatekeeperURL" => $gatekeeperURL,
			]);
		}
