...
function ossearch_get_gatekeeperURL($args = [])
{
	...
	if (!empty($gatekeeperURL)) {
		$gatekeeperURL = preg_match("#https?://#", $gatekeeperURL)
			? $gatekeeperURL
			: "http://" . $gatekeeperURL;
	}
...
function dir_popular_query($method_name, $params, $app_data)
{
	...
	$query_start = $req["query_start"];
	$include_hypergrid =
		isset($req["include_hypergrid"]) && $req["include_hypergrid"] == "false"
			? false
			: true;
	$include_landsales =
		isset($req["include_landsales"]) && $req["include_landsales"] == "true"
			? true
			: false;
	...
	if (!$include_landsales) {
		$left_join =
			"LEFT JOIN parcelsales AS sales ON sales.parcelUUID = pop.parcelUUID";
	...
	INNER JOIN ' .
		SEARCH_REGION_TABLE .
		' as r ON par.regionUUID = r.regionUUID
	' .
		$left_join .
		'
	' .
...
function dir_land_query($method_name, $params, $app_data)
{
	...
	$sql =
		"SELECT *,saleprice/area AS lsq FROM parcelsales $where ORDER BY " .
		$order .
...
function event_info_query($method_name, $params, $app_data)
{
	...
	while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
		...
		// debug($row);
		if (
			isset($_REQUEST["gk"]) & !empty($_REQUEST["gk"]) &&
			$_REQUEST["gk"] == $row["gatekeeperURL"]
		) {
...
function classifieds_info_query($method_name, $params, $app_data)
{
	...
	$query = $OpenSimDB->prepare(
		"SELECT * FROM classifieds WHERE classifieduuid = ?",
	);
	$result = $query->execute([$classifiedID]);
