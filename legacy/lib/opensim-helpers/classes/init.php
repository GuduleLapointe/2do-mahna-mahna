...
class OpenSim
{
	...
	public function db_connect()
	{
		...
		$connectionstring = self::get_option(
	...
	public function get_helpers_url()
	{
		...
		$url_path = self::trailingslashit(
			str_replace($_SERVER["DOCUMENT_ROOT"], "", $helpers_path),
		);
	...
	public static function get_version($sanitized = false)
	{
		...
		if (file_exists(".git/HEAD")) {
			...
			if (!empty($hash) && file_exists(".git/refs/heads/" . $hash)) {
				$hash =
					substr(
						file_get_contents(".git/refs/heads/" . $hash),
						0,
						7,
					) . " ($hash)";
			} else {
	...
	public static function connectionstring_to_array($connectionstring)
	{
		...
		if (preg_match('/:[0-9]+$/', $creds["Data Source"])) {
			...
			$creds["Port"] = empty($host[1] || $host[1] == 3306)
				? null
				: $creds["Port"];
		}
		$result = [
	...
	public static function enqueue_script(
		...
		$handle = preg_match("/^oshelpers-/", $handle)
			? $handle
			: "oshelpers-" . $handle;
		$handle = rtrim($handle, "-css") . "-js";

		self::$scripts = self::$scripts ?? ["head" => [], "footer" => []];
		if (strpos($src, "://") === false) {
			$src = OSHELPERS_URL . ltrim($src, "/");
		}
		$ver = empty($ver)
			? self::get_version(true)
			: self::sanitize_slug($ver);
		$src = self::add_query_args($src, ["ver" => $ver]);

		$section = $in_footer ? "footer" : "head";
		self::$scripts[$section][$handle] = [
			"src" => $src,
			"deps" => $deps,
	...
	public static function enqueue_style(
		...
		self::$styles = self::$styles ?? ["head" => [], "footer" => []];
		if (strpos($src, "://") === false) {
			$src = OSHELPERS_URL . ltrim($src, "/");
		}
		$ver = empty($ver)
	...
	public static function get_styles($echo = false)
	{
		...
		$template =
			'<link id="%s" rel="stylesheet" href="%s" type="text/css" media="%s">';
	...
	{
		$config = $_SESSION["installation"]["config"] ?? (self::$config ?? []);
		if (empty($config)) {
			return $default;
		}
