...
2do-aggregator
├── *.md
├── cron.sh		// Full update task for cron — assembles public/ and rsyncs to targets
├── package.json
├── aggregator.php
├── assets/		// Static resources (images, fonts) — never auto-generated
├── build/		// Compiled front-end assets (.min.css, .min.js)
├── cache/
├── config/		// Host-specific config files, only modify *.example files
├── dev/		// Scripts and files needed only during development
├── exporters/	// Output converters
├── includes/	// Files included in one or more scripts
├── logs/
├── parsers/	// Input converters
├── public/		// Auto-generated rsync target (web root), DO NOT EDIT
├── src/		// Editable source files (PHP, HTML, CSS, JS, LSL)
├── tests/		// Test units
├── tmp/		// Store any temporary data here
└── vendor/
```
