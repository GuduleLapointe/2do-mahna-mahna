...
if (!$SearchDB || !$SearchDB->connected) {
	...
	die("500 Internal Server Error: SearchDB not available\n");
}
...
switch ($service) {
	case "online":
		...
		$query = $SearchDB->prepare(
			"SELECT register FROM hostsregister WHERE host = :host AND port = :port",
		);
		$query->execute([
			":host" => $host,
			":port" => $port,
      ...
      WHERE host = :host AND port = :port',
			...
			$query = $SearchDB->prepare(
				"INSERT INTO hostsregister VALUES (:host, :port, :timestamp, 0, 0, 0, '')",
			);
