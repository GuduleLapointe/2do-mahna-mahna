...
function currency_xmlrpc_buy($method_name, $params, $app_data)
{
	...
	$ret = false;
	$cost = currency_virtual_to_real($amount);
	$transactionPermit = currency_process_transaction(
		$agentid,
		$cost,
		$ipAddress,
	);
	...
	if ($ret) {
		$response_xml = xmlrpc_encode(["success" => true]);
	} else {
		switch (CURRENCY_PROVIDER) {
			case "podex":
			...
			case "gloebit":
				...
				$errorMessage =
					"Click OK to finish the transaction on Gloebit website.";
				break;

			default:
				$errorMessage =
					"Unable to process the transaction. The gateway denied your charge. Open help page?";
				$errorURI = empty(W4OS_GRID_INFO["help"])
					? CURRENCY_HELPER_URL
					: W4OS_GRID_INFO["help"];
		}
		$response_xml = xmlrpc_encode([
...
xmlrpc_server_register_method(
	$xmlrpc_server,
	"simulatorUserBalanceRequest",
	"currency_xmlrpc_balance",
);
function currency_xmlrpc_balance($method_name, $params, $app_data)
{
	...
	if ($balance >= 0) {
		...
		$response_xml = xmlrpc_encode([
			"success" => false,
			"errorMessage" =>
				"Could not authenticate your avatar. Money operations may be unavailable",
			"errorURI" => " ",
		]);
	}
...
xmlrpc_server_register_method(
	$xmlrpc_server,
	"regionMoveMoney",
	"currency_xmlrpc_regionMoveMoney",
);
	...
	if ($ret) {
		...
		if ($ret) {
			...
			if ($balance >= $cash) {
				...
				currency_update_simulator_balance(
					$agentid,
					$sbalance,
					$sessionid,
				);
			...
			$response_xml = xmlrpc_encode([
				"success" => false,
				"errorMessage" =>
					"Unable to authenticate avatar. Money operations may be unavailable",
				"errorURI" => " ",
			]);
		}
	} else {
		$response_xml = xmlrpc_encode([
			"success" => false,
			"errorMessage" =>
				"This region is not authorized to manage your money.",
			"errorURI" => " ",
		]);
	}
...
xmlrpc_server_register_method(
	$xmlrpc_server,
	"simulatorClaimUserRequest",
	"currency_xmlrpc_claimUserRequest",
);
