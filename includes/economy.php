...
if (defined("CURRENCY_DB_HOST")) {
	$CurrencyDB = new OSPDO(
		"mysql:host=" . CURRENCY_DB_HOST . ";dbname=" . CURRENCY_DB_NAME,
		CURRENCY_DB_USER,
		CURRENCY_DB_PASS,
	);
} else {
...
function noserver_save_transaction(
	$sourceId,
	$destId,
	$amount,
	$type,
	...
	$CurrencyDB->insert(CURRENCY_TRANSACTION_TBL, [
		"sourceId" => $sourceId,
		"destId" => $destId,
		"amount" => $amount,
		"flags" => $flags,
...
function currency_save_transaction(
	$sourceId,
	$destId,
	$amount,
	$type,
...
function currency_update_simulator_balance(
	$agentID,
	$amount = -1,
	$secureID = null,
) {
	...
	$request = xmlrpc_encode_request("UpdateBalance", [
		[
			"clientUUID" => $agentID,
...
function currency_add_money($agentID, $amount, $secureID = null)
{
	...
	$request = xmlrpc_encode_request("AddBankerMoney", [
		[
			"clientUUID" => $agentID,
			"clientSessionID" => $sessionID,
			"clientSecureSessionID" => $secureID,
...
function currency_send_money($agentID, $amount, $secretCode = null)
{
	...
	$request = xmlrpc_encode_request("SendMoneyBalance", [
		[
			"clientUUID" => $agentID,
...
function currency_get_balance($agentID, $secureID = null)
{
	...
	$request = xmlrpc_encode_request("GetBalance", [
		[
			"clientUUID" => $agentID,
			"clientSessionID" => $sessionID,
			"clientSecureSessionID" => $secureID,
...
function opensim_get_avatar_session($agentID, &$deprecated = null)
{
	...
	$result = $OpenSimDB->query(
		"SELECT RegionID,SessionID,SecureSessionID FROM Presence WHERE UserID='$agentID'",
	);
	if ($result) {
		[$RegionID, $SessionID, $SecureSessionID] = $result->fetch();
	} else {
		return [];
	}
...
function opensim_get_server_info($userid, &$deprecated = null)
{
	...
	if ($result) {
		[$serverip, $httpport, $serveruri, $secret] = $result->fetch();
	} else {
		return [];
	}
...
function opensim_check_secure_session(
	$agentID,
	$regionid,
	$secure,
	&$deprecated = null,
