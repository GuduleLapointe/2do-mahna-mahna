...
switch ($method) {
	case "/SaveMessage/":
		...
		if ($emailinfo) {
			[$sendmail, $email] = $emailinfo->fetch();
			...
			$headers =
				"From: $xml->fromAgentName <" . OPENSIM_MAIL_SENDER . ">\r\n";
			...
			$body =
				...
				OPENSIM_GRID_NAME .
				"</b> Instant Messages mail forwarding by w4os." .
				...
				add_action(
					...
					function () use ($email, $subject, $body, $headers) {
						...
						if (!$result) {
							error_log(
								__FILE__ .
									"error $result sending IM notification to $email.",
							);
						}
						die();
					},
	...
	case "/RetrieveMessages/":
		if (opensim_isuuid($xml->Guid)) {
			...
			if ($pendingmessages) {
				...
				while ([$id, $message] = $pendingmessages->fetch()) {
	...
	default:
		error_log(
			"Offline messages: method $method not implemented, please configure OfflineMessageModule = OfflineMessageModule in OpenSim.ini",
		);
