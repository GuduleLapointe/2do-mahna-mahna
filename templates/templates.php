...
{
	$id = "navbar-$slug";
	$html = sprintf('<div class="collapse navbar-collapse" id="%s">', $id);
	...
	foreach ($menu as $key => $item) {
		if (
			isset($item["condition"]) &&
			!OpenSim::validate_condition($item["condition"])
		) {
			continue;
		}
...
$branding =
	...
	htmlspecialchars($GLOBALS["site_title"]) .
	"</a>";
