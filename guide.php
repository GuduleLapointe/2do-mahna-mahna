...
class OpenSim_Guide
{
	...
	public function __construct($source = null)
	{
		...
		if (!empty($source)) {
			...
			defined("OPENSIM_GUIDE_SOURCE") &&
			!empty(OPENSIM_GUIDE_SOURCE)
		) {
	...
	public function build_html()
	{
		...
		if (count($this->destinations) === 1) {
			...
			$category = isset($this->destinations[$category])
				? $category
				: null;
		}
		...
		foreach ($lines as $line) {
			...
			if (isset($parts[3]) && trim($parts[3]) !== "") {
				...
				$url =
					trim($parts[3]) .
					"/" .
					(isset($parts[4]) ? trim($parts[4]) : null);
			} else {
			...
			if (empty($url)) {
				...
				$this->destinations[$categoryTitle][] = [
					"name" => $name,
					"url" => $url,
				];
			}
	...
	public function categories_list()
	{
		...
		foreach ($this->destinations as $categoryTitle => $destinations) {
			if (!empty($destinations)) {
				$content .=
					'<a href="' .
					$this->build_url($categoryTitle) .
					'">' .
					'<div class="item">' .
					'<img class="thumbnail" src="' .
					$this->place_thumbnail() .
					'" alt="' .
					$categoryTitle .
					'">' .
					'<div class="name">' .
	...
	public function destinations_list($categoryTitle)
	{
		...
		foreach ($this->destinations[$categoryTitle] as $destination) {
			...
			$content .=
				'<a href="' .
				opensim_format_tp($destination["url"], TPLINK_HG) .
				'">' .
				'<div class="item">' .
				'<img class="thumbnail" src="' .
				$this->place_thumbnail() .
			...
			if ($people > 0) {
				$content .=
					" <span>" .
					sprintf(_("%s people"), $this->place_people()) .
					"</span> ";
			}
			if ($traffic > 0) {
				$content .=
					" <span>" .
					sprintf(_("traffic %s"), $this->place_traffic()) .
					"</span> ";
			}
			$content .= "</div></div></a>";
		}
	...
	private function no_result()
	{
		$content =
			'<div class="error">' .
			_(
				"The realm of destinations you seek has eluded our grasp, spirited away by elusive knomes. Rally the grid managers, let them venture forth to curate a grand tapestry of remarkable places for your exploration!",
			) .
			"</div>";
		return $content;
	}
	...
	private function is_main_script()
	{
		...
		if (function_exists("debug_backtrace")) {
			...
			foreach ($trace as $trace_info) {
				if (
					isset($trace_info["function"]) &&
					in_array($trace_info["function"], ["include", "require"])
				) {
					return false;
				}
			}
