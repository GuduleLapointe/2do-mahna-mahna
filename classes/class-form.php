...
class OpenSim_Form
{
	...
	public function __construct($args = [], $step = 0)
	{
		if (is_string($args)) {
		...
		if (!is_array($args)) {
			...
			throw new InvalidArgumentException(
				"Invalid argument type: " . gettype($args),
			);
		}

		$args = OpenSim::parse_args($args, [
		...
		$this->completed =
			$_SESSION[$this->form_id]["completed"] ?? $this->completed;
	 ...
	 */
	public static function register($args = [], $step = 0)
	{
		try {
	...
	public function render_form()
	{
		...
		$reset_button = empty($_SESSION[$form_id])
			? ""
			: sprintf(
		...
		foreach ($fields as $field => $data) {
			...
			if (!empty($this->errors[$field])) {
				...
				$data["help"] =
					OpenSim::error_html($field_error, "warning") .
					($data["help"] ?? "");
	...
	public function get_values()
	{
		...
		foreach ($fields as $key => $field) {
			$values[$key] =
				$_POST[$key] ??
				($_SESSION[$form_id][$key] ?? ($field["value"] ?? null));
		}
		return $values;
	}
	...
	{
		if (
			!isset($this->next_step_key) ||
			!isset($this->fields[$this->next_step_key])
		) {
			return [];
		}
	...
	private function refresh_steps()
	{
		...
		if (!empty($_POST["form_id"])) {
			...
			if ($form) {
				...
				error_log(
					__METHOD__ . " Form " . $form_id . " is not registered",
				);
				return false;
			}
		}
	...
	public function is_valid_ini_file($field_id, $file_path, $strict = false)
	{
		...
		if ($strict) {
			...
			if (empty($ini)) {
				$message = sprintf(
					_("File %s does not comply with .ini standards."),
					$file_path,
				);
				$this->task_error($field_id, $message, "danger");
		...
		if (!empty($valid)) {
			$message = sprintf(
				_("File %s is not a valid .ini file"),
				$file_path,
			);
			error_log(
				$message . ", found invalid lines: " . print_r($valid, true),
			);
			$this->task_error($field_id, $message, "danger");
			return false;
		}
	...
	public function is_robust_ini_file($field_id, $file_path)
	{
		...
		if (!empty($missing)) {
			$message = sprintf(
				_("Not a valid Robust config file."),
				$file_path,
				implode(", ", $missing),
			);
			$this->task_error($field_id, $message, "danger");
			$message = sprintf(
				_("%s is missing required sections: %s"),
				$file_path,
