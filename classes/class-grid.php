...
class OpenSim_Grid
{
	...
	public static function get_grid_info($grid_uri = false, $args = [])
	{
		// global $oshelpers_cache;

		$key = __METHOD__ . md5(serialize(func_get_args()));
		$grids_info = os_cache_get("grids_info", []);
	...
	public static function array_to_card($id, $info, $args = [])
	{
		if (empty($info)) {
			return;
		}
		...
		if (!empty($args["title"]) && is_string($args["title"])) {
			...
			if (
				is_numeric(array_keys($info)[0]) ||
				(isset($args["hide_first"]) && $args["hide_first"] === true)
			) {
	 ...
	 */
	public static function grid_info_card($grid_uri = false, $args = [])
	{
		...
		return self::array_to_card("grid-info", $info, [
			"title" => $title,
		]);
	}
	...
	public static function grid_stats_card($args = null)
	{
		...
		return self::array_to_card("grid-status", $grid_stats, [
			"title" => $title,
		]);
	}
	...
	public static function get_grid_stats($args = null)
	{
		...
		if (!$robust_db || OpenSim::is_error($robust_db)) {
			...
			$stats = [
				...
				"active_members" => $robust_db->get_var(
					"SELECT COUNT(*)
				...
				"active_users" => $robust_db->get_var(
					"SELECT COUNT(*)
				...
				"regions" => $robust_db->get_var("SELECT COUNT(*)
                    FROM regions"),
				"total_area" =>
					$robust_db->get_var("SELECT round(sum(sizex * sizey / 1000000),2)
                    FROM regions") . "&nbsp;km²",
			];
